...
/* 
 * File:   a2vectorTest.cpp
 * Author: Иван Губочкин
 *
 * Created on 15.08.2012, 21:37:00
 */
...
void testA2V()
{
    std::cout << "a2vectorTest testA2V" << std::endl;
    ...
    try
    {
        ...
        aam::AAMFunctions2D::appearance2Vector(grayImage, vertices,
                basePoints,
                objectPixels, nCount, triangles, J, grayVector);
        std::cout << "appearance2Vector working time " <<
        ...
        {
            std::cout << "%TEST_FAILED% time=0 testname=testA2V (a2vectorTest) message=Invalid output texture size" << std::endl;
        }
        else if (J.channels() != 1)
        {
            std::cout << "%TEST_FAILED% time=0 testname=testA2V (a2vectorTest) message=Invalid output texture channels count" << std::endl;
        }
        else if (grayVector.cols != 1 || grayVector.rows != 52314)
        {
            std::cout << "%TEST_FAILED% time=0 testname=testA2V (a2vectorTest) message=Invalid output vector size" << std::endl;
        }
    }
    catch (std::exception& e)
    {
        std::cout << "%TEST_FAILED% time=0 testname=testA2V (a2vectorTest) message=Exception occured: " <<
                e.what() << std::endl;
    }
}
...
{
    std::cout << "%SUITE_STARTING% a2vectorTest" << std::endl;
    std::cout << "%SUITE_STARTED%" << std::endl;

    std::cout << "%TEST_STARTED% testA2V (a2vectorTest)" << std::endl;
    testA2V();
    std::cout << "%TEST_FINISHED% time=0 testA2V (a2vectorTest)" << std::endl;

    std::cout << "%TEST_STARTED% testA2VColor (a2vectorTest)\n" << std::endl;
    testA2VColor();
    std::cout << "%TEST_FINISHED% time=0 testA2VColor (a2vectorTest)" << std::endl;
