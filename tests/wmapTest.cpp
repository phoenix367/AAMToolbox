...
TEST(WarpMap, testGetWarpMap)
{
    ...
    pathStream << AAM_TEST_DATA_PATH << "/warp_map.bmp";
    cv::Mat1i targetMap = cv::imread(pathStream.str(),
            cv::IMREAD_GRAYSCALE);
    ...
    EXPECT_EQ(warpMap.size, targetMap.size);
    EXPECT_EQ(cv::countNonZero(warpMap - (targetMap - 1)), 0);
}
