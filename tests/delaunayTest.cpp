...
TEST(Delaunay, testDelaunay)
{
    ...
    EXPECT_EQ(result.size(), TRIANGLES_NUMBER);
    // Check for duplicates
    std::vector<cv::Vec3i>::iterator endSeqIt = std::unique(
            result.begin(), result.end());
    EXPECT_EQ(endSeqIt - result.begin(), TRIANGLES_NUMBER);

    for (int i = 0; i < result.size(); i++)
    {
        EXPECT_TRUE(checkTriangle(result[i], targetTrinagles));
    }
}

TEST(Delaunay, testEmpty)
{
