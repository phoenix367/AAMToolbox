...
static aam::RealType verticesAData[2 * TOTAL_POINTS_NUMBER] =
{
   309,   227,
   326,   228,
   348,   238,
...
TEST(AlignData, testAlign)
{
    ...
    EXPECT_EQ(result.size(), TOTAL_POINTS_NUMBER);
    EXPECT_LE(fabs(tform.offsetSX - 1.0087528652811386), 1e-6);
    EXPECT_LE(fabs(tform.offsetSY - 0.011903690443852661), 1e-6);
    EXPECT_LE(fabs(targetOffsetV.x - tform.offsetV.x), 1e-6);
    EXPECT_LE(fabs(targetOffsetV.y - tform.offsetV.y), 1e-6);
    for (int i = 0; i < result.size(); i++)
    {
        EXPECT_LE(fabs(result[i].x - targetVertices[i].x), 1e-6);
        EXPECT_LE(fabs(result[i].y - targetVertices[i].y), 1e-6);
    }
}

TEST(AlignData, testAlignInverse)
{
    aam::TForm tform;
